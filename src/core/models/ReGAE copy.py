...
class FTTransformerEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor | None]:
        if x.ndim != 3:
            raise ValueError(
...
class FTTransformerDecoder(nn.Module):
    """
    표준 decoder 형태(마스킹/시프트 없이 autoencoder 용도로 사용):
      - self-attn (decoder tokens)
      - cross-attn (Q=decoder tokens, K/V=encoder memory tokens)
      - FFN
    """
    ...
    def forward(self, x: Tensor, memory: Tensor) -> Dict[str, Tensor]:
        if x.ndim != 3:
        ...
        for block in self.blocks:
            ...
            att = block["cross_att"](x, memory)  # Q=decoder, K/V=encoder memory
...
class FTTransformer(nn.Module):
    def __init__(
        ...
        **backbone_kwargs: Any,
        ...
        # decoder 입력을 "값과 무관한 learnable query embedding"으로 만들기 위한 파라미터
        # (F, D) -> forward에서 (B, F, D)로 expand
    ...
    def forward(
        ...
        # -----------------------
        # 1) encoder용 feature embeddings (B, F, D)
        #    (encoder는 값 기반으로 token을 만들어야 하므로 x_cont/x_cat로부터 생성)
        # -----------------------
        ...
        if self.cont_embeddings is None:
            ...
            feat_tokens.append(self.cont_embeddings(x_cont))  # (B, n_cont, D)
        ...
        if self.cat_embeddings is None:
            ...
            feat_tokens.append(self.cat_embeddings(x_cat))  # (B, n_cat, D)
        ...
        # -----------------------
        # 2) encoder input: CLS + features (B, 1+F, D)
        # -----------------------
        x_cls = self.cls_embedding(x_any.shape[:-1])  # (B, 1, D)
        x_enc = torch.cat([x_cls, x_feat_enc], dim=1)  # (B, 1+F, D)

        enc = self.encoder(x_enc)
        mem = enc["tokens"]
        if mem is None:
            raise ValueError("encoder must return tokens as memory")

        # -----------------------
        # 3) decoder input: 값과 무관한 learnable query embedding (B, F, D)
        #    encoder memory는 cross-attn K/V로 매 레이어 전달
        # -----------------------
        B = int(x_enc.shape[0])
        x_dec_in = self.dec_queries.unsqueeze(0).expand(B, -1, -1)  # (B, F, D)

        dec = self.decoder(x_dec_in, cast(Tensor, mem))
        tokens_dec = dec["tokens"]  # (B, F, D)

        recon = None
        if self.recon_cont_head is not None:
            s = 0
            e = self.n_cont_features
            cont_tokens = tokens_dec[:, s:e, :]  # (B, n_cont, D)
            recon = self.recon_cont_head(cont_tokens).squeeze(-1)  # (B, n_cont)

        return {
            "logits": enc["logits"],
            "embedding": enc["embedding"],
            "tokens_enc": enc["tokens"],

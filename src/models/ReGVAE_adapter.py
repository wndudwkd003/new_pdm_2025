...
class ReGVAEAdapter(BaseModelAdapter):
    def __init__(self, config: Config):
        ...
        # stage1 weights (loss module 내부에서 사용)
    ...
    def fit(self, train_data: Datasets, valid_data: Datasets):
        ...
        # -------------------------
        # Train memory bank export (clean only)
        # -------------------------
        root = (
            self.work_dir
        ...
        # -------------------------
        # Stage1 embedding visualization (valid missing views)
        # -------------------------
        ...
        # -------------------------
        # Stage 2 (retrieval + feat gate + CE)
        # -------------------------
        ...
        _, tr_preds, tr_labels, _, _ = self.predict(
            tr_loader, split=Split.TRAIN, stage=2, memory_path=mem_path
        )
        _, vl_preds, vl_labels, _, _ = self.predict(
            vl_loader, split=Split.VALID, stage=2, memory_path=mem_path
        )
    ...
    def test(self, test_data: Datasets):
        ...
        loss, preds_all, labels_all, pattern_idx_all, ratio_idx_all = self.predict(
            te_loader, split=Split.TEST, stage=2, memory_path=mem_path
        )

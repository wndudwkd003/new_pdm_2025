...
class MLPAdapter(BaseModelAdapter):
    ...
    def _infer_input_dim_output_dim(
        ...
        # classification: num_class 추론(가장 신뢰: y)
        ...
        # meta.num_class가 있으면 일치 검증(있을 수도, 없을 수도)
    ...
    def _pred_loss(self, pred: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        regression: MSE
        classification(binary/multiclass): CrossEntropy
        """
    ...
    def _pred_to_output(self, pred: torch.Tensor) -> torch.Tensor:
        """
        regression: (B,1)->(B,)
        classification: logits -> argmax labels (B,)
        """
        if self.is_regression:
    ...
    def predict(
        ...
        for batch in tqdm(loader, desc=desc):
            ...
            # pattern/ratio는 연산에 안 쓰므로 device 여부 상관없지만, numpy로 합치기 위해 tensor 보장
    ...
    # model
    # -------------------------
    def _get_model(self, input_dim: int, output_dim: int | None) -> MLP:
        if output_dim is None:
            raise ValueError("output_dim is None")

        # 필요하면 config로 빼도 됩니다. (요청하시면 config 연동 버전으로 바꿔드리겠습니다)
        n_blocks = 4
        d_block = 128
        dropout = 0.1
